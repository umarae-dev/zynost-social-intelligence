...
- List runtime deps needed for HTTP and Redis **clients** (`httpx` for providers; `redis` for `cache.py`) — never `anthropic`, `openai`, `google.generativeai`, or other LLM SDKs (`CON-01`, **TST-LLM**).

...
- List runtime deps needed for HTTP and Redis **clients** (`httpx` for X, Reddit, and Telegram; Redis client later) — never `anthropic`, `openai`, `google.generativeai`, or other LLM SDKs (`CON-01`, **TST-LLM**).
